...
const bossFunc = (dataSources) => {
  ...
  let counter = subordinates.length

  // control flow defined here
  const callback = (err, result) => {
    if (err) { handleError(err) }
    if (result.text) {
      publishReport(writeReport(result.text)) //don't wait to publish!
    }
  ...
  forEachIndexed((subordinate, i)  => {
    subordinate(dataSources[i], callback)
  }, subordinates)
